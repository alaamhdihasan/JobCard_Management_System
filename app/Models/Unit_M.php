...
class Unit_M extends Model
{
    ...
    var $column_order = array(null, 'Ui_Name'); //set column field database for datatable orderable
    var $order = array('Ui_ID' => 'asc'); // default order 

    function searchAndDisplay($usersdata = null, $start = 0, $length = 0, $order = null)
    {
        $builder = $this->table("tb_Unit");
        if ($usersdata) {
            $arr_usersdata = explode(" ", $usersdata);
            for ($x = 0; $x < count($arr_usersdata); $x++) {
                $builder = $builder->orLike('Ui_Name', $arr_usersdata[$x]);
            }
        }
